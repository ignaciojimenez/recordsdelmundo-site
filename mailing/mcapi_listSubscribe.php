<?php
/**
This Example shows how to Subscribe a New Member to a List using the MCAPI.php 
class and do some basic error checking.
**/

require_once 'MCAPI.class.php';
require_once 'config.inc.php'; //contains apikey

$api = new MCAPI($apikey);

/*$merge_vars = array('FNAME'=>'Test', 'LNAME'=>'Account', 
                  'GROUPINGS'=>array(
                        array('name'=>'Your Interests:', 'groups'=>'Bananas,Apples'),
                        array('id'=>22, 'groups'=>'Trains'),
                        )
                    );
*/
// By default this sends a confirmation email - you will not see new members
// until the link contained in it is clicked!
$subscriberemailID = $_GET['email'];
$merge_vars = null;
$retval = $api->listSubscribe( $listId, $subscriberemailID, $merge_vars );

if ($api->errorCode){
	//echo "Unable to load listSubscribe()!\n";
	//echo "\tCode=".$api->errorCode."\n";
	//echo "\tMsg=".$api->errorMessage."\n";
	if ($api->errorCode == 214){
		print_r($subscriberemailID . " ya existe!");
	}else{
		echo "Ha habido un problema al darte de alta!\n";
		echo "Intentalo de nuevo m&aacute;s tarde!\n";
	}
} else {
    print_r($subscriberemailID . " dado de alta");
}

?>
